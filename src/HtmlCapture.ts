...
export class HtmlCapture {
	...
	fontEmbedCSSForElement(element: HTMLElement): string {
		...
		const filtered = filterFontBlocksForElement(this._fontBlocks, element);
		return filtered.map((b) => b.css).join('\n');
	}
	...
	async captureElement(
		...
		if (cacheIsFresh) {
			if (!force) return;
			// Throttle dynamic recaptures to at most once per 100ms
			if (now - cached.lastCaptureTime < 100) return;
		}
		...
		try {
			await this._captureWithHtmlToImage(element, w, h, cssW, cssH, forBackground);
		} finally {
		...
		try {
			const rendered = await toCanvas(element, {
				...
				filter: (node: any) => {
					...
					if (hideSet && hideSet.has(node)) return false;
					...
					if (node.classList && (
						...
						node.classList.contains('customizer-drawer') ||
						node.classList.contains('dialog-backdrop')
					)) {
						return false;
					}
					return true;
	...
	/** Invalidate the entire capture cache. */
	clear(): void {
		this.cache.clear();
	}
		...
		try {
			const rendered = await toCanvas(element, {
				...
				filter: (node: any) => {
					...
					// Background captures keep glass shells for layout but strip
					// their DOM content so on-glass text is never refracted/blurred.
					if (forBackground) {
						if (node.tagName === 'GLASS-PANEL' || node.tagName === 'GLASS-BUTTON') {
							return true;
						}
						...
						while (parent && parent !== element) {
							if (parent.tagName === 'GLASS-PANEL' || parent.tagName === 'GLASS-BUTTON') {
								return false;
							}
			...
			this.cache.set(element, { canvas: rendered, w, h, lastCaptureTime: performance.now() });
